...
#' set.seed(42)
#' error_loo_boot(x = iris_x, y = iris_y, train = MASS:::lda, classify = lda_wrapper)
#' # Output: 0.02307171
error_loo_boot <- function(x, y, train, classify, num_bootstraps = 50, ...) {
  x <- as.matrix(x)
  y <- as.factor(y)
  ...
  loo_boot_error_rates <- lapply(seq_len(num_bootstraps), function(b) {
    ...
    classify_obj <- train(x[training, ], y[training])
    classifications <- classify(object = classify_obj, newdata = x[test, ])
