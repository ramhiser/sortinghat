...
#' errorest(x = iris_x, y = iris_y, train = MASS:::lda, predict = lda_wrapper)
#'
#' # .632
...
#'          predict = lda_wrapper, num_bootstraps = 42)
#' 
errorest <- function(x, y, estimator = c("cv", "boot", "632", "632+", "bcv",
                               "loo-boot", "apparent"), train, predict, ...) {
  x <- as.matrix(x)
  y <- as.factor(y)
  ...
  switch(estimator,
    cv = errorest_cv(x = x, y = y, train = train, predict = predict, ...),
    boot = errorest_boot(x = x, y = y, train = train, predict = predict, ...),
    `632` = errorest_632(x = x, y = y, train = train, predict = predict, ...),
    `632+` = errorest_632plus(x = x, y = y, train = train, predict = predict, ...),
    bcv = errorest_bcv(x = x, y = y, train = train, predict = predict, ...),
    `loo-boot` = errorest_loo_boot(x = x, y = y, train = train, predict = predict, ...),
