...
#' @param predict a function that classifies observations from the classifier
...
#' errorest_apparent(x = iris_x, y = iris_y, train = MASS:::lda, predict = lda_wrapper)
#' # Output: 0.02
#' 
...
#' # Output: 0.02
errorest_apparent <- function(x, y, train, predict, ...) {
  x <- as.matrix(x)
  y <- as.factor(y)
