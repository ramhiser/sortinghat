#' Generate mixture (non-Gaussian) data in the format described in Skurichina and Duin (1998).
#'
#' @export
#' @param n sample size of population
#' @param p dimension of the generated data
#' @param seed the random number seed of the data.
generate_duin <- function(n = 100, p = 10, seed = NULL) {
  if(!is.null(seed)) set.seed(seed)

  # There are two populations in this generated data set.
  num_groups <- 2
	
  # First we generate two multivariate normal samples that share a
  # common covariance matrix but have different means.
